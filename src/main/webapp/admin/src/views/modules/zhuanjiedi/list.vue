        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zhuanjiedi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xuelizhengming/upload/zhuanjiediMuBan.xls"
                        >批量导入转接地数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zhuanjiedi','导入导出')"
                                ...
                                :on-success="zhuanjiediUploadSuccess"
                                :on-error="zhuanjiediUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zhuanjiedi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zhuanjiedi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zhuanjiediAddress"
                                   header-align="center"
                                   label="转接地地址">
                        <template slot-scope="scope">
                            {{scope.row.zhuanjiediAddress}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('zhuanjiedi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zhuanjiedi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('zhuanjiedi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                zhuanjiediUuidNumber : null,
                zhuanjiediName : null,
                zhuanjiediAddress : null,
                zhuanjiediTypes : null,
                zhuanjiediErjiTypes : null,
                zhuanjiediContent : null,
                shangdianShouyinContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zhuanjiedi',//当前表表名,
                        sumColum : 'zhuanjiedi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zhuanjiedi_name',//分组字符串字段
                        // types : 'zhuanjiedi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['zhuanjiediDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            zhuanjiediUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zhuanjiedi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
